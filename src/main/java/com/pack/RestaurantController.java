...
public class RestaurantController extends HttpServlet {
    ...
    public String selectAdd(HttpServletRequest request, HttpServletResponse response, @RequestParam("a") Integer id)
    {
        String del_add="";
        String del_name="";
        try 
         {
             ...
             Class.forName("com.mysql.cj.jdbc.Driver");
             Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/fooddelivery?characterEncoding=utf8", "root", "root");
             PreparedStatement stmt=con.prepareStatement("select * from deliver_address where del_id=?");
             stmt.setInt(1, id);
             
            ResultSet rst = stmt.executeQuery();

            if (rst.next()) {
                del_name=rst.getString("name");
                del_add=rst.getString("address");
                
                session.setAttribute("DeliveryAddress", del_add);
                session.setAttribute("DeliveryName", del_name);
            }             
         }
        catch (Exception k)
        {
    ...
    @RequestMapping(value = "/restro", method = RequestMethod.GET)
    public String restroDetails()
    {
        return "restro";
    }
}